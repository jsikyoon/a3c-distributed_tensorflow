...
class GameACLSTMNetwork(GameACNetwork):
  def __init__(self,
    ...
    with tf.device(self._device), tf.variable_scope(scope_name) as scope:
      ...
      self.W_fc1, self.b_fc1 = self._fc_variable([STATE_SIZE*4, 200])
      # lstm
      self.lstm = tf.contrib.rnn.BasicLSTMCell(200, state_is_tuple=True)
      # weight for policy output layer
      self.W_fc2, self.b_fc2 = self._fc_variable([200, action_size])
      self.W_fc3, self.b_fc3 = self._fc_variable([200, action_size])
      # weight for value output layer
      ...
      h_fc1_reshaped = tf.reshape(h_fc1, [1,-1,200])
      # h_fc_reshaped = (1,5,256)

      # place holder for LSTM unrolling time step size.
      self.step_size = tf.placeholder(tf.float32, [1])

      self.initial_lstm_state0 = tf.placeholder(tf.float32, [1, 200])
      self.initial_lstm_state1 = tf.placeholder(tf.float32, [1, 200])
      ...
      # lstm_outputs: (1,5,200) for back prop, (1,1,200) for forward prop.
      
      lstm_outputs = tf.reshape(lstm_outputs, [-1,200])
      # policy (output)
      self.mu = tf.matmul(lstm_outputs, self.W_fc2) + self.b_fc2
      self.sigma = tf.matmul(lstm_outputs, self.W_fc3) + self.b_fc3
      # value (output)
  ...
  def reset_state(self):
    self.lstm_state_out = tf.contrib.rnn.LSTMStateTuple(np.zeros([1, 200]),
                                                        np.zeros([1, 200]))

  def run_policy_and_value(self, sess, s_t):
    # This run_policy_and_value() is used when forward propagating.
    # so the step size is 1.
    action_out, v_out, self.lstm_state_out = sess.run( [self.action, self.v, self.lstm_state],
  ...
  def run_policy(self, sess, s_t):
    ...
    action_out, self.lstm_state_out = sess.run( [self.action, self.lstm_state],
                                            feed_dict = {self.s : [s_t],
                                                         self.initial_lstm_state0 : self.lstm_state_out[0],
                                                         self.initial_lstm_state1 : self.lstm_state_out[1],
                                                         self.step_size : [1]} )
